...
class MRImageFlow(object):
	...
	def InputSelectionCallback(self, caller, event):
		...
		if annSel.GetNumberOfNodes() > 0:
			...
			if idxVtk.GetNumberOfTuples() > 0:
				...
				self.imStack = self.ds.GetMultiResolutionImages(idxArr.tolist()[0])
			...
			i_range = N.array(self.imStack.GetPointData().GetArray(i_array_name).GetRange())
			...
			for ii in range(self.numImages):
				...
				# TODO: Here need to set opacity of actor based on some weights
	...
	def selectImage(self, caller, event):
	...
	def ImageFlowSelectionChanged(self):
		...
		id_vtk = VN.numpy_to_vtkIdTypeArray(id_array)
...
if __name__ == "__main__":
	...
	if_class = MRImageFlow(ds, dummy_link)
...
#  	if_class.SetFlowDirection(Direction.Horizontal)
	...
	id_array = N.array([0,1,2,3,4,5],dtype='int64')
